...
use pyo3::prelude::*;
use snarkvm::console::algorithms::Poseidon2 as Poseidon2Native;
use snarkvm::prelude::{Hash, HashMany, HashToGroup, HashToScalar};

/// Poseidon2 is a cryptographic hash function with an input rate of 2.
#[pyclass(frozen)]
pub struct Poseidon2(Poseidon2Native<CurrentNetwork>);

#[pymethods]
impl Poseidon2 {
    /// Creates a Poseidon2 hasher with the default domain separator "AleoPoseidon2".
    #[new]
    pub fn new() -> anyhow::Result<Self> {
        Poseidon2Native::setup("AleoPoseidon2").map(Self)
    }

    /// Creates a Poseidon2 hasher with a custom domain separator.
    #[staticmethod]
    pub fn setup(domain_separator: &str) -> anyhow::Result<Self> {
        Poseidon2Native::setup(domain_separator).map(Self)
    }
    ...
    /// Returns multiple Poseidon2 hash outputs for the given field elements.
    ...
    /// Returns the Poseidon2 hash of the given field elements projected to a scalar.
    pub fn hash_to_scalar(&self, input: Vec<Field>) -> anyhow::Result<Scalar> {
        let input: Vec<FieldNative> = input.into_iter().map(FieldNative::from).collect();
        self.0.hash_to_scalar(&input).map(Scalar::from)
    }

    /// Returns the Poseidon2 hash of the given field elements projected to a group element.
