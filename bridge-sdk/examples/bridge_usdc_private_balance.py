"""Bridge USDC to a private USDCx balance without a separate recipient claim.

Choose Public Bridge to Private Balance when the recipient needs funds for
private use on Aleo. The delivered record is encrypted, but the Ethereum
deposit still reveals the Aleo address, Ethereum sender, and amount.
The sender needs USDC and ETH for fees; run without --execute to review a quote.
"""
import os
import sys
...
def main(argv=None):
    ...
    quote = bridge.quote(
        ...
        mint_mode="record",  # Receive an encrypted record automatically; the Aleo address stays visible.
    )
