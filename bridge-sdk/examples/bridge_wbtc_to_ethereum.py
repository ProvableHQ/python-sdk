"""Return WBTC held on Aleo to an Ethereum address.

Use this transfer to make WBTC available to Ethereum applications. Spend a
public Aleo WBTC balance; unshield private WBTC first. Keep Aleo credits for
transaction and Hyperlane delivery fees. Receiving WBTC needs no Ethereum key.
"""
import os
import sys
...
def main(argv=None):
    parser = outbound_parser(__doc__, amount="0.001")
    ...
    quote = bridge.quote(
        source_chain="aleo", source_asset="wbtc",
        destination_chain="ethereum", destination_asset="wbtc",
        amount=args.amount,  # Display units of WBTC, not atomic units.
        recipient=args.recipient,
        sender=bridge.aleo_address() if args.execute else args.sender,
    )
    print("Expected received:", quote.amount_out)  # WBTC on Ethereum, in display units.
