...
def test_evm_hyperlane_quote_dispatches_to_eth_and_carries_the_canonical_plan():
    b = FakeBridge()
    q = quote(b, source_chain="ethereum", source_asset="wbtc", destination_chain="aleo", destination_asset="wbtc", amount="0.001", recipient=ALEO_RECIPIENT,
              sender=EVM_ADDRESS)
...
def test_evm_xreserve_quote_passes_mint_mode_and_secret_nonce():
    b = FakeBridge()
    q = quote(b, source_chain="ethereum", source_asset="usdc", destination_chain="aleo", destination_asset="usdcx", amount="2", recipient=ALEO_RECIPIENT,
...
def test_solana_hyperlane_quote():
    ...
    q = quote(b, source_chain="solana", source_asset="sol", destination_chain="aleo", destination_asset="sol", amount="0.000000001", recipient=ALEO_RECIPIENT,
              sender=SOL_ADDRESS)
...
def test_aleo_hyperlane_quote_reads_the_igp_only():
    ...
    q = quote(b, source_chain="aleo", source_asset="eth", destination_chain="ethereum", destination_asset="eth", amount="0.000000000000000001",
...
def test_aleo_xreserve_quote_is_offline_and_deducts_the_withdrawal_fee():
    b = FakeBridge(ethereum=False)
    q = quote(b, source_chain="aleo", source_asset="usdcx", destination_chain="ethereum", destination_asset="usdc", amount="2.000001", recipient=EVM_ADDRESS)
    ...
    with pytest.raises(InvalidAmountError, match="exceed the 2 USDCx withdrawal fee"):
        quote(b, source_chain="aleo", source_asset="usdcx", destination_chain="ethereum", destination_asset="usdc", amount="2", recipient=EVM_ADDRESS)


def test_missing_connection_and_unavailable_route():
    b = FakeBridge(ethereum=False)
    with pytest.raises(ConfigurationError, match="Ethereum connection"):
        quote(b, source_chain="ethereum", source_asset="usdc", destination_chain="aleo", destination_asset="usdcx", amount="2", recipient=ALEO_RECIPIENT)
    with pytest.raises(RouteUnavailableError, match="metadata-required"):
        quote(b, source_chain="aleo", source_asset="aleo", destination_chain="ethereum", destination_asset="aleo", amount="1", recipient=EVM_ADDRESS)


def test_quote_takes_chain_asset_keywords_or_a_route():
    b = FakeBridge()
    route = b.registry.route("hyperlane:ethereum/wbtc->aleo/wbtc")
    by_filters = quote(b, source_chain="ethereum", source_asset="wbtc", destination_chain="aleo",
                       amount="0.001", recipient=ALEO_RECIPIENT)
    by_route = quote(b, route=route, amount="0.001", recipient=ALEO_RECIPIENT)
    by_route_id = quote(b, route=route.id, amount="0.001", recipient=ALEO_RECIPIENT)
    assert by_filters.plan == by_route.plan == by_route_id.plan
    assert by_filters.plan.route_id == route.id
    with pytest.raises(TypeError):
        quote(b, "ethereum/wbtc", "aleo/wbtc", amount="0.001", recipient=ALEO_RECIPIENT)   # keyword-only
