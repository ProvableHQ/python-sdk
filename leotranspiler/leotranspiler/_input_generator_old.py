...
class _InputGenerator:
    ...
    def format_struct(self, input_dict, leo_type):
        if isinstance(input_dict, dict):
            return (
                "{"
                + ", ".join(
                    [
                        f"{key}: {self.format_struct(value, leo_type)}"
    ...
    def generate_input(self, fixed_point_features):
        ...
        # Build the nested input dictionary
        nested_input_dict_list = []
        current_dict = {}
        current_count = 0

        for input in self.input_list:
            if input.active:
                if current_count >= self.MAX_CIRCUIT_INPUTS:
                    nested_input_dict_list.append(current_dict)
                    current_dict = {}
                    current_count = 0

                keys = input.reference_name.split(".")
                d = current_dict
                for key in keys[:-1]:
                    d = d.setdefault(key, {})
                d[keys[-1]] = input.get_set_value()
                current_count += 1

        if current_dict:
            nested_input_dict_list.append(current_dict)

        # Format the nested input dictionary
        formatted_input_list = [
            self.format_struct(d, "i64") for d in nested_input_dict_list
        ]  # assuming all are i64 for simplicity
